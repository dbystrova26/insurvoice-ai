...
def assess_urgency(intent: str, turn_count: int, handoff_summary: str) -> str:
    ...
    FIX [9]: Added billing-specific distress keywords so billing escalations
    are not always rated 'low'. Previously 'overcharge', 'refused', 'wrong amount'
    etc. were missing, causing all billing escalations to show Priority: low in Slack.
    """
    ...
    medium_urgency_keywords = [
        ...
        # FIX [9]: billing distress words missing from original
        "wrong amount", "incorrect charge", "double charged", "refund",
        "not paying", "dispute", "error on bill", "billing error",
        "premium too high", "never received", "not authorised",
    ]
...
def generate_call_summary(conversation_history: list, intent: str, resolved: bool) -> str:
    ...
    return (
        f"Customer contacted InsurVoice AI regarding {topic}. "
...
def fire_n8n_webhook(
    ...
    Called for ALL turns (not just escalations) so Google Sheets
    logging is complete. The n8n SWITCH node handles branching.
    Returns True if webhook was queued, False if no URL configured.
    """
    ...
    def _send():
        try:
            log.info("[n8n] Sending webhook to: %s...", N8N_WEBHOOK_URL[:60])
            log.info(
                "[n8n] Payload - Call ID: %s, Intent: %s, Escalated: %s",
                call_id, intent, escalated,
            )
            r = requests.post(
                N8N_WEBHOOK_URL,
                json=payload,
            ...
            if r.status_code not in (200, 201):
                log.error("[n8n] Webhook returned %s: %s", r.status_code, r.text[:100])
            else:
                log.info("[n8n] ✅ Webhook sent successfully. Call ID: %s", call_id)
        except Exception as e:
            log.error("[n8n] ❌ Webhook error: %s", e)
