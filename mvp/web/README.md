...
InsurVoice AI uses **Retrieval-Augmented Generation (RAG)** to ground Tina's responses in actual insurance policy documents instead of relying solely on LLM training data.
...
Customer: "Does my insurance cover water damage?"
    ↓
retrieve_context(query, top_k=2)
    ├─ Load 154 FAQs from knowledge_base.json
    ├─ Score by keyword overlap
    └─ Return top 2 FAQs
    ↓
FAQ Context: 
    "Q: Does water damage have a deductible?
     A: EUR 250 per claim event.
    
     Q: What counts as water damage?
     A: Burst pipes, washing machines, plumbing fixtures..."
    ↓
Build Claude Prompt with FAQ context
    ↓
Claude API generates response grounded in policy
    ↓
Tina: "Yes, water damage from burst pipes is covered with a EUR 250 
       deductible. This covers water escaping from pipes, washing machines,
       and plumbing fixtures inside your home..."
```
...
**Code:** `mvp/web/knowledge.py:retrieve_context()`
